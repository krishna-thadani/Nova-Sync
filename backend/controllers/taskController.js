...
export const updateTask = async (req, res) => {
  try {
    ...
    // If the task was just marked completed, and it was due before today, pay off past debt!
    if (updates.status === "Completed" && updatedTask.dueDate) {
      const todayStart = new Date();
      todayStart.setHours(0, 0, 0, 0);
      
      // If due in the past
      if (updatedTask.dueDate < todayStart) {
        // Find oldest unsettled debt
        import("../src/models/DebtRecord.js").then(async ({ default: DebtRecord }) => {
           const oldestDebt = await DebtRecord.findOne({ 
             userId, 
             isSettled: false,
             $expr: { $lt: ["$repaidTasks", "$taskDebt"] } 
           }).sort({ date: 1 });
           
           if (oldestDebt) {
             oldestDebt.repaidTasks += 1;
        ...
        }).catch(err => console.error("Error updating debt record for past task:", err));
      }
    }
    // ----------------------------
